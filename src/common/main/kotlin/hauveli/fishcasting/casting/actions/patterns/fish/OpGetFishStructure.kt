...
*/
object OpGetFishStructure : ConstMediaAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
        ...
        val mediumCondition = definitelyFish.conditions().stream()
            .filter { obj: FishingCondition -> obj is FishingMediumCondition }
            .map { obj: FishingCondition -> obj as FishingMediumCondition }
            .findFirst()
            .orElse(null)

        // I think this is true?
        if (mediumCondition == null) {
            return listOf(MediumIota(MediumIota.Medium.Water.ordinal))
        }

        return listOf(MediumIota(MediumIota.Medium.of(mediumCondition.mediumId).ordinal))
    }
}