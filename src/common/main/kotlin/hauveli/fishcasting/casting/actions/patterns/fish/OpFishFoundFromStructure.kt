...
*/
object OpFishFoundFromStructure : ConstMediaAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
        ...
        val someIota = args[1]
        if (someIota !is StructureIota) {
            throw MishapInvalidIota(
                someIota,
                1,
                StructureIota.translation("testing this")
            )
        }
        ...
        val structure = someIota.value

        val foundInMoonPhase = relevantConditions.all { condition ->
            when (condition) {
                is StructuresCondition -> {
                    condition.structures.contains(structure)
                }
