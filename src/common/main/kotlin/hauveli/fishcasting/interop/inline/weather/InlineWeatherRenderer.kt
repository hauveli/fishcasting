...
class InlineWeatherRenderer : InlineRenderer<InlineWeatherData> {
    override fun getId(): ResourceLocation {
        return InlineWeatherData.rendererId
    }

    override fun getGlowPreference(forData: InlineWeatherData?): GlowHandling {
        return GlowHandling.None()
    }

    // mostly static
    private val WEATHER_TYPES = Tide.resource("textures/gui/journal/weather.png")
    ...
    override fun charWidth(data: InlineWeatherData?, style: Style?, codepoint: Int): Int {

        return 0 // must be ZERO or it's FUCKED
    }

    companion object {
        val INSTANCE: InlineWeatherRenderer = InlineWeatherRenderer()
    }
}