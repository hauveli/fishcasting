...
class InlineDaytimeRenderer : InlineRenderer<InlineDaytimeData> {
    override fun getId(): ResourceLocation {
        return InlineDaytimeData.rendererId
    }

    override fun getGlowPreference(forData: InlineDaytimeData?): GlowHandling {
        return GlowHandling.None()
    }

    // mostly static
    private val DEPTH_BAR = Tide.resource("textures/gui/journal/time_bar.png")
    ...
    override fun render(
        ...
        // -35c to +50c
        val u = Mth.clamp(
            Mth.inverseLerp(data.daytime.mod(24000).toDouble(), 0.0, 24000.0),
            0.0, 1.0).toFloat() // 1 and 0 are the same, I think
        val v = 0f
    ...
    override fun charWidth(data: InlineDaytimeData?, style: Style?, codepoint: Int): Int {

        return 0 // must be ZERO or it's FUCKED
    }

    companion object {
        val INSTANCE: InlineDaytimeRenderer = InlineDaytimeRenderer()
    }
}